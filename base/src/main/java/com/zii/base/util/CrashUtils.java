...
public class CrashUtils {
  ...
  static {
    ...
    UNCAUGHT_EXCEPTION_HANDLER = new Thread.UncaughtExceptionHandler() {
      ...
      public void uncaughtException(final Thread t, final Throwable e) {
        ...
        final String crashInfo = getCrashInfo(e, time);
        createLogFile(crashInfo, time);

        delayExitApp();

        // MARK mark-zii : 2018/11/11  不回调原来的处理，需要日志这调用回调去上传
  ...
  private static void createLogFile(String crashInfo, String time) {
  ...
  private static void delayExitApp() {
    new Thread(new Runnable() {
      @Override
      public void run() {
        Looper.prepare();
        Toast.makeText(Utils.getApp(), "哎呀，程序出现了一点意外，即将退出", Toast.LENGTH_SHORT).show();
        Looper.loop();
        try {
          Thread.sleep(2000);
